    ...
    $('#form_edit').on('show.bs.modal', function (e) {
        ...
        DataManager.fetchData(detail.replace(':id', id))
            .then(function (response) {
                if (response.success) {
                    $('#edit_libur_nasional_id').val(response.data.libur_nasional_id);
                    $('#edit_nama_libur_nasional').val(response.data.nama_libur_nasional);
        ...
        $('#bt_submit_edit').on('submit', function (e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    const input = {
                        libur_nasional_id: $('#edit_libur_nasional_id').val(),
                        nama_libur_nasional: $('#edit_nama_libur_nasional').val(),
                        tanggal: $('#edit_tanggal').val(),


                    };
                    const update = '{{ route('admin.absensi.libur_nasional.update', [':id']) }}';
